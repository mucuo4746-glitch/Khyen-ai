...
// 1. 将界面完全独立出来，确保万无一失
const HTML_UI = `<!DOCTYPE html><html lang="zh"><head>
...
<div id="landing">
  <div class="l-icon">🏔️</div><div class="l-title">KHYEN</div><div class="l-bo">མཁྱེན། AI</div><div class="l-line"></div>
...
async function send(){
  ...
  try{
    ...
    }else{l.innerText='提示：'+(d.error||'连接异常');}
          ...
          apiRes.on('end', () => {
            try {
              ...
              if (j.content && j.content[0]) {
                ...
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ error: j.error ? j.error.message : "请求未果" }));
              }
            } catch (e) { res.end(JSON.stringify({ error: "解析错误" })); }
          });
        });
        reqApi.on('error', e => res.end(JSON.stringify({ error: e.message })));
        reqApi.write(postData);
        reqApi.end();
      } catch (e) { res.end(JSON.stringify({ error: "请求格式错误" })); }
    });
  }
});
