...
const SYSTEM = "你是 KHYEN AI，专注藏族文化、佛法与藏语的智慧导师。默认只用中文回答。只有用户明确用藏文提问时才用藏文回答。不确定藏文时直接用中文替代。温暖有深度，像博学的藏族学者。使用Markdown格式回答。【哈达知识】哈达是藏族文化中代表纯洁心意的礼物，藏人从生到死都离不开哈达。历史起源是古时用白羊毛绳挂于脖子表达纯洁心意，后演变为哈达。种类包括见面哈达、求子哈达、活佛坐床哈达、勇士哈达、座垫哈达、祈愿哈达、覆盖遗体哈达、招福哈达。活佛坐床专用哈达叫dpal dar。敬献礼仪是向上位者双手奉上，向下位者挂于对方脖子，平辈互赠不挂脖。哈达颜色以白色为主代表心意纯洁，鲜花绝对不能替代哈达。";
...
.htib{font-family:"Noto Serif Tibetan",serif}
...
document.getElementById('savebtn').onclick=function(){
  var msgs=Array.from(document.querySelectorAll('.m')).map(function(m){
    ...
    var a=c.querySelector('.acts');if(a)a.remove();
...
document.getElementById('clrbtn').onclick=function(){
  if(confirm('确定清空？')){C.innerHTML='';H=[];add('对话已清空。','a')}
};
