...
.m{max-width:88%;padding:16px 20px;border-radius:16px;line-height:1.8;box-shadow:0 2px 8px rgba(0,0,0,0.06)}
...
async function send(){
  ...
  try{
    ...
    } else { loader.innerText = '错误: ' + (data.error || '解析失败'); }
...
document.getElementById('save-btn').onclick=()=>{
  ...
  a.download='Khyen_Chat.txt';
  a.click();
  showToast('已保存！');
};
document.getElementById('clear-btn').onclick=()=>{
  if(confirm('清空对话？')){chat.innerHTML='';h=[];add('对话已清空。','a');}
};
document.getElementById('sb').onclick=send;
document.getElementById('t').onkeydown=e=>{
        ...
        req.on('end', async () => {
            try {
                ...
                const postData = JSON.stringify({
                    ...
                    max_tokens: 4096,
                    system: "你是 KHYEN AI，专注藏族文化、佛法与藏语的智慧导师。请用温暖、深度的口吻回答，Markdown格式。",
                    messages: messages
                });
                const reqApi = https.request({
                    ...
                    apiRes.on('end', () => {
                        ...
                        } catch(e) { res.end(JSON.stringify({ error: "API响应解析失败" })); }
                    });
                });
                reqApi.write(postData); reqApi.end();
            } catch(e) { res.end(JSON.stringify({ error: "后端请求错误" })); }
        });
    }
});
