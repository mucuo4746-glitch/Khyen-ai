...
const server = http.createServer((req, res) => {
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        /* 深度优化：包含藏文时的样式 */
        .tibetan-mode { font-family: 'Noto Serif Tibetan', serif !important; line-height: 2.5 !important; font-size: 19px !important; }
    ...
    <body>
        ...
        <script>
            ...
            function saveChat() {
                const msgs = Array.from(document.querySelectorAll('.m')).map(m => m.innerText.replace('📋 复制', '')).join('\\n\\n---\\n\\n');
            ...
            function clearChat() {
                if(confirm('确定清空对话记录？')) { c.innerHTML = ''; h = []; add('对话已清空。', 'a'); }
            }
            function add(msg, type) {
                ...
                if(type === 'a') {
                    ...
                    copyBtn.className = 'action-btn'; copyBtn.innerText = '📋 复制';
        ...
        req.on('end', async () => {
            try {
                ...
                const postData = JSON.stringify({
                    ...
                    max_tokens: 4096,
                    system: "你是 KHYEN AI མཁྱེན།，专注藏族文化、佛法与藏语的智慧导师。1. 默认只用中文回答。2. 只有提问包含藏文时才用藏文回答。3. 藏文必须包含正确音节点且单独成行。4. 遇到佛法问题请多引用《入菩萨行论》等经典。5. 保持温暖、睿智、谦虚的藏族学者语气。",
                    messages: messages
                });
