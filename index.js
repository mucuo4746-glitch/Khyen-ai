...
const server = http.createServer((req, res) => {
    if (req.url === '/' || req.url === '/index.html') {
        ...
        <style>
            body { font-family: 'Noto Serif Tibetan', serif; background: #fdfbf7; margin: 0; display: flex; flex-direction: column; height: 100vh; color: #3d2b1f; -webkit-font-smoothing: antialiased; }
            ...
            .m { max-width: 85%; padding: 14px 18px; border-radius: 18px; line-height: 2.6; font-size: 19px; word-wrap: break-word; box-shadow: 0 2px 8px rgba(0,0,0,0.05); font-variant-ligatures: discretionary-ligatures; text-rendering: optimizeLegibility; }
        ...
        req.on('end', async () => {
            try {
                ...
                const postData = JSON.stringify({
                    ...
                    max_tokens: 2048,
                    system: "你叫 KHYEN AI མཁྱེན།。是一位睿智、谦虚的导师。请始终保持藏汉双语回答。使用 Markdown 格式。",
                    messages: messages
                });
