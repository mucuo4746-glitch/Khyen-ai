        ...
        res.end(`
            ...
            <head>
                ...
                <style>
                    ...
                    /* 背景巨轮 */
                    #chat-box::before {
                        content: "☸"; position: fixed; top: 45%; left: 50%; 
                    ...
                    .speak-btn { display: inline-block; margin-top: 10px; font-size: 18px; color: var(--gold); cursor: pointer; opacity: 0.6; }

                    .input-container { padding: 15px 20px 30px; background: white; border-top: 1px solid #f5f5f5; }
                    .input-area { max-width: 600px; margin: 0 auto; display: flex; gap: 12px; align-items: center; }
                    input { flex: 1; background: #f7f7f7; border: 1px solid #eee; padding: 12px 20px; border-radius: 25px; outline: none; font-size: 16px; }
                    button { background: var(--dark); color: var(--gold); border: 1px solid var(--gold); padding: 10px 22px; border-radius: 25px; font-weight: bold; cursor: pointer; }
                </style>
            </head>
            <body>
...
server.listen(PORT, () => { console.log('Khyen 纯粹版已就绪'); });
